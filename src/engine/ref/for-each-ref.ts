 ...
 * 在视图首帧渲染时可能尚未填充（异步），且其元素是 `Ref`（无 upstream / 无 HEAD 标记 / 无 ahead-behind）。
 * 故改用受控 CLI 通道执行 `git for-each-ref`，一次拿全字段（含 upstream / ahead-behind track / HEAD 标记），
 * 供 adapter 渲染与 Phase 1 的 favorites / ahead-behind 展示复用。
 *
...
export const FOR_EACH_REF_FORMAT =
	'%(refname)%00%(refname:short)%00%(objectname:short)%00%(upstream:short)%00%(upstream:track)%00%(HEAD)';
...
export interface RawRef {
	...
	/** 领先上游的提交数（来自 %(upstream:track) 的 "[ahead N]"）。 */
	readonly ahead?: number;
	/** 落后上游的提交数（来自 %(upstream:track) 的 "[behind M]"）。 */
...
const NUL = '\x00';
const AHEAD_RE = /\bahead\s+(\d+)/;
const BEHIND_RE = /\bbehind\s+(\d+)/;

/**
 * 解析 `git for-each-ref`（NUL 分隔 6 字段）输出为 RawRef[]。
...
export function parseForEachRef(output: string): RawRef[] {
	...
	for (const line of output.split('\n')) {
		...
		const aheadMatch = track.match(AHEAD_RE);
		const behindMatch = track.match(BEHIND_RE);
		refs.push({
			refname,
			shortName,
			objectname,
