# Hyper Git v0.0.5 — 首个 MVP 正式版

> 在 VS Code 上完整复刻 IntelliJ IDEA 的 **Git 工具窗口** 与 **Commit 提交窗口**，并为未来 AI Agent 自主代理预留架构接缝。

这是 Hyper Git 的**首个对外正式版本**。在经历了若干内部迭代与 `v0.0.1-rc.*` 预发布打磨后，我们以 **v0.0.5** 作为 MVP 正式版对外首发——把 IDEA 里那套被开发者高频依赖、迁移到 VS Code 后却长期缺失的 Git 工作流**一次性补齐**：多 Changelist 变更组织、忠实的提交窗口、可视化提交图、Shelf、行级提交，并与原生 Source Control **平行共存、零冲突**。
...
- **IDEA 风格提交图（自绘 DAG）× CI 状态**：彩色泳道、分叉·合并连线、HEAD/分支/标签徽标；每条提交行右侧实时显示 **GitHub CI 最终状态**（绿勾/红叉/运行中），悬停查看各项检查、失败原因与跳转链接——告别 `git log --graph` 的粗糙 ASCII，也无需切到浏览器查构建结果。
...
每条提交行最右侧显示其 **CI 最终状态**：绿勾=通过、红叉=失败、运行中=黄色旋转。悬停图标以浮层 Tooltip 展示「各项检查 + 未通过原因 + 运行链接」，效果对标 IntelliJ IDEA / GitHub，让你**在提交图里一眼看出哪次提交挂了 CI**，无需跳转浏览器。
...
> 规模实证：**7 个视图 / 93 个命令 / 6 个配置项 / 280 个单元测试**，覆盖 IDEA Git 工具窗口主线工作流。
...
- **手动安装（当前推荐）**：从 [Releases](https://github.com/ThreeFish-AI/hyper-git/releases) 下载 `hyper-git-0.0.5.vsix` → 命令面板执行 `Extensions: Install from VSIX`。
