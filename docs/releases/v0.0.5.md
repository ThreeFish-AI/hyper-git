...
这是 Hyper Git 的**首个对外正式版本**。在经历了若干内部迭代与 `v0.0.1-rc.*` 预发布打磨后，我们以 **v0.0.5** 作为 MVP 正式版对外首发——把开发者高频依赖、却在 VS Code 原生 Source Control 中长期缺失的 Git 工作流**一次性补齐**：多 Changelist 变更组织、自绘提交面板、可视化提交图、Shelf、行级提交，并与原生 Source Control **平行共存、零冲突**。
