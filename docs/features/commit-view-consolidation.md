...
| Changelist 管理 | 标题栏 `$(checklist)` 图标（Refresh 左侧）→ `hyperGit.setActiveChangelist` QuickPick：各列表带文件计数、当前活动项 `picked` 预选，分隔线后并入 New / Rename / Delete…（默认列表不可改名删除）；活动列表名常驻 `view.description` 副标题、Default 活动列表时省略（原头部 `<select>` + `⋯` 菜单行已上移，省一行竖直空间） |
| Git 操作工具栏 | 标题栏（`view/title`）承接 refresh / push / pull / fetch / pushDialog / updateProject / createPatch / applyPatch；「…」菜单 Discard Changes 批量撤销勾选文件（`discardChanges` 单/多统一路径）；去掉与 webview 内 Commit·Commit&Push 按钮重复的两项 |
...
Commit 视图文件区以**活动 Changelist** 为提交目标（勾选集即提交范围）；非活动 Changelist 通过标题栏 `$(checklist)` 图标弹 QuickPick 切换，活动列表名常驻 `view.description` 副标题（Default 省略）。相较旧树「同屏并列多 changelist」少了一处低频便利，换取窄侧栏下更聚焦的提交工作流。
...
`pnpm run check-types && pnpm run lint && pnpm run test:unit` 全绿；Extension Development Host（F5）：无 CHANGES 视图；标题栏 `$(checklist)` 图标可切换/管理 changelist、副标题显示活动列表名（Default 时无副标题）；文件单击开 Diff、右键出菜单（含 discard 确认）；「…」菜单 Discard Changes 按 modal 确认批量撤销勾选文件（未跟踪删除、已跟踪还原，无勾选时仅提示）；标题栏 Git 动作可用；活动栏角标随未提交数增减；无 Git 仓库时降级不崩溃。
