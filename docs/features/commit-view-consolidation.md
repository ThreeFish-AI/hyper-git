...
| Changelist 管理 | 标题栏「…」菜单 → Set Active Changelist… → `hyperGit.setActiveChangelist` QuickPick：各列表带文件计数、当前活动项 `picked` 预选，分隔线后并入 New / Rename / Delete…（默认列表不可改名删除）；活动列表名常驻 `view.description` 副标题、Default 活动列表时省略（原头部 `<select>` + `⋯` 菜单行已上移，省一行竖直空间） |
| Git 操作工具栏 | 标题栏（`view/title`）承接 refresh / push / pull / fetch / pushDialog / updateProject / createPatch / applyPatch；最左 `$(discard)` 图标 Discard Changes 批量撤销勾选文件（`discardChanges` 单/多统一路径）；New Changelist… 收入「…」菜单；去掉与 webview 内 Commit·Commit&Push 按钮重复的两项 |
...
Commit 视图文件区以**活动 Changelist** 为提交目标（勾选集即提交范围）；非活动 Changelist 通过标题栏「…」菜单 Set Active Changelist… 弹 QuickPick 切换，活动列表名常驻 `view.description` 副标题（Default 省略）。相较旧树「同屏并列多 changelist」少了一处低频便利，换取窄侧栏下更聚焦的提交工作流。
...
- 命令：[`adapter/commands.ts`](../../src/adapter/commands.ts)（`resolveChange` + 签名重构；`setActiveChangelist` QuickPick 承载切换与管理；`discardChanges` 单/多统一路径：归一数组 + modal 批量确认 + clean/restore 各一次调用）。
- 装配：[`extension.ts`](../../src/extension.ts)（移除 tree/changesView，角标改由 `commitView.updateBadge`）。
- 协议：[`shared/protocol.ts`](../../src/shared/protocol.ts)（`commit/openFile|fileMenu|checkedChanged`；changelist 操作消息已随标题栏上移移除，`checkedChanged` 为勾选集 webview → host 单向镜像，供标题栏批量 Discard）。

## 验证

`pnpm run check-types && pnpm run lint && pnpm run test:unit` 全绿；Extension Development Host（F5）：无 CHANGES 视图；标题栏最左 `$(discard)` 图标按 modal 确认批量撤销勾选文件（未跟踪删除、已跟踪还原，无勾选时仅提示）；「…」菜单可切换/管理 changelist（Set Active Changelist… QuickPick）与新建列表、副标题显示活动列表名（Default 时无副标题）；文件单击开 Diff、右键出菜单（含 discard 确认）；标题栏 Git 动作可用；活动栏角标随未提交数增减；无 Git 仓库时降级不崩溃。
